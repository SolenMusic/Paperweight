...
constexpr std::array<Showcase, 59> showcases{{
    ...
    {"Illuminated Science Fiction", "illuminated-scifi"},
...
std::uint64_t checksum(const paperweight::MaterialImageSet& images)
{
    std::uint64_t hash = 1469598103934665603ULL;
    for (const auto output : paperweight::materialOutputs) {
        const auto* image = images.image(output);
        if (image == nullptr) {
            continue;
        }
...
bool imagesIdentical(
    const paperweight::MaterialImageSet& left,
    const paperweight::MaterialImageSet& right)
{
    for (const auto output : paperweight::materialOutputs) {
        const auto* leftImage = left.image(output);
        const auto* rightImage = right.image(output);
        if ((leftImage == nullptr) != (rightImage == nullptr)) {
            return false;
        }
        if (leftImage != nullptr && !std::equal(
                leftImage->pixels().begin(), leftImage->pixels().end(),
                rightImage->pixels().begin(), rightImage->pixels().end())) {
            return false;
        }
    }
    return true;
...
- (void)buildInterface
{
    ...
    NSTextField* explanation = labelWithText(
        @"Runs every bundled material at 64, 128, 256, 512, and 1024 pixels. "
         "Each result compares one worker with the full worker pool and verifies every generated byte exactly.");
    ...
    [self.outputPopup addItemsWithTitles:@[
        @"Complete 10-map material set",
        @"Colour", @"Height", @"Normal", @"Roughness", @"Metalness",
        @"Coating", @"Occlusion", @"Clear coat", @"Coat roughness", @"Emissive",
    ]];
    self.outputPopup.toolTip =
        @"Benchmark a complete coordinated material generation or one output map.";
    ...
    self.completedSummary = labelWithText([NSString stringWithFormat:
        @"Completed: 0 / %zu", showcases.size() * resolutions.size()]);
...
- (void)startBenchmark:(id)sender
{
    ...
    completeSetSelected_ = self.outputPopup.indexOfSelectedItem == 0;
    selectedOutput_ = completeSetSelected_
        ? paperweight::MaterialOutput::colour
        : static_cast<paperweight::MaterialOutput>(self.outputPopup.indexOfSelectedItem - 1);
    ...
    self.runMetadata = [NSString stringWithFormat:
        ...
        memory,
        completeSetSelected_ ? @"Complete 10-map material set" : outputDisplayName(selectedOutput_)];
    ...
    const bool completeSet = completeSetSelected_;
    ...
    dispatch_async(benchmarkQueue_, ^{
        @autoreleasepool {
            for (const auto& [materialName, path] : materialFiles) {
                ...
                outputs[paperweight::materialOutputIndex(output)] = true;
                if (completeSet) {
                    outputs = paperweight::allMaterialOutputsSelected;
                }
                paperweight::MaterialSetRequest request;
                ...
                for (const auto resolution : resolutions) {
                    ...
                    auto serialResult = paperweight::generateMaterialSet(request, [cancellation]() {
                    ...
                    auto* serialImages = std::get_if<paperweight::MaterialImageSet>(&serialResult);
                    if (serialImages == nullptr) {
                    ...
                    auto parallelResult = paperweight::generateMaterialSet(request, [cancellation]() {
                        return cancellation->load(std::memory_order_relaxed);
                    });
                    ...
                    auto* parallelImages = std::get_if<paperweight::MaterialImageSet>(&parallelResult);
                    if (parallelImages == nullptr) {
                        NSString* message = [NSString stringWithFormat:
                    ...
                    const double megapixels = static_cast<double>(resolution) * resolution /
                        1'000'000.0 * (completeSet ? paperweight::materialOutputs.size() : 1);
                    ...
                    const bool identical = imagesIdentical(*serialImages, *parallelImages);
                    const auto resultChecksum = checksum(*serialImages);
...
- (NSString*)csvText
{
    ...
    NSString* output = completeSetSelected_
        ? @"Complete 10-map material set"
