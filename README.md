...
v0.0.9 is the current Performance release. It adds reproducible release
benchmarks, golden hashes for every showcase output, an evaluator execution
plan, and deterministic multithreaded CPU generation. Native builds select a
bounded worker count automatically for larger images; callers can force the
serial reference path, and builds can disable threading entirely. The material
graph, `.pmat` definitions, and generated bytes are unchanged. The native app's
Tools > Performance Benchmark window compares one-worker and multi-worker
generation across every bundled material and five output resolutions, presents
the results live, verifies exact pixel equality, and exports portable CSV.
...
The 3D material preview and stable game library follow in the documented
roadmap. Optional Vulkan/MoltenVK research remains separate from the CPU-first
release and from ordinary `libpaperweight` builds.
