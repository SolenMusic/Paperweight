...
std::string downgradeCurrentPmat(std::string text, std::uint32_t version)
{
    const std::string current =
        "pmat.version = " + std::to_string(paperweight::currentPmatVersion);
    const auto versionPosition = text.find(current);
    if (versionPosition != std::string::npos) {
        text.replace(
            versionPosition,
            current.size(),
            "pmat.version = " + std::to_string(version));
    }
    if (version < 23) {
        constexpr std::string_view groupCount = "groups.count = 0\n";
        const auto groupPosition = text.find(groupCount);
        if (groupPosition != std::string::npos) {
            text.erase(groupPosition, groupCount.size());
        }
    }
    if (version >= 18) {
        return text;
    ...
    for (const std::string_view prefix : {
        ...
        if (position != std::string::npos) {
            ...
            text.erase(position, end == std::string::npos
                    ? text.size() - position
                    : end - position + 1);
        }
    }
    for (const std::string_view suffix : {
        ...
        while ((position = text.find(suffix, position)) != std::string::npos) {
            text.erase(position, suffix.size());
        }
    }
    return text;
}
...
void testVersion()
{
    constexpr paperweight::Version expected{0, 0, 33};
    static_assert(paperweight::currentVersion == expected);
    expect(paperweight::versionString() == "0.0.33", "version string is 0.0.33");
}
...
void testLayerFragments()
{
    ...
    expect(text != nullptr &&
               text->find("paperweight.layer_fragment.version = 2") != std::string::npos,
    ...
    if (text != nullptr) {
        ...
        auto legacy = *text;
        const auto legacyFragmentMarker = legacy.find("layer_fragment.version = 2");
        legacy.replace(
            legacyFragmentMarker,
            std::string_view{"layer_fragment.version = 2"}.size(),
            "layer_fragment.version = 1");
        legacy = downgradeCurrentPmat(std::move(legacy), 22);
        const auto legacyDecoded = paperweight::parseLayerFragment(legacy);
        expect(std::holds_alternative<paperweight::LayerFragment>(legacyDecoded),
               "v0.0.31 version-1 clipboard fragments remain readable");

        auto future = *text;
        const auto marker = future.find("layer_fragment.version = 2");
        future.replace(marker, std::string_view{"layer_fragment.version = 2"}.size(),
...
void testLayerGroupsAndReusableOverlays()
{
    paperweight::Material material;
    material.seed = 320032;
    material.physicalSize = {1.0, 1.0};

    auto background = paperweight::makeSolidColourLayer({30, 34, 39, 255});
    background.outputs = {};
    auto groupBase = paperweight::makeSolidColourLayer({74, 112, 58, 255});
    groupBase.outputs = {};
    auto detail = paperweight::makeNoiseLayer(731);
    detail.opacity = 0.55;
    detail.outputs = {};
    material.layers = {background, groupBase, detail};

    auto passThrough = material;
    passThrough.layerGroups = {
        paperweight::MaterialLayerGroup{
            "group.outer", {}, "Outer", true, 1.0,
            paperweight::CompositeMode::blend, {}, {}, {}},
        paperweight::MaterialLayerGroup{
            "group.inner", "group.outer", "Inner", true, 1.0,
            paperweight::CompositeMode::blend, {}, {}, {}},
    };
    passThrough.layerHierarchy = {
        {"layer.background", {}},
        {"layer.group.base", "group.outer"},
        {"layer.group.detail", "group.inner"},
    };
    for (const auto output : paperweight::materialOutputs) {
        const auto flatResult = paperweight::generate(
            {material, 29, 23, output, std::nullopt, std::nullopt, 1});
        const auto groupedResult = paperweight::generate(
            {passThrough, 29, 23, output, std::nullopt, std::nullopt, 1});
        const auto* flat = std::get_if<paperweight::Image>(&flatResult);
        const auto* grouped = std::get_if<paperweight::Image>(&groupedResult);
        expect(flat != nullptr && grouped != nullptr &&
                   std::equal(flat->pixels().begin(), flat->pixels().end(),
                              grouped->pixels().begin()),
               "default nested grouping preserves every output byte");
    }

    material.layerGroups = {
        paperweight::MaterialLayerGroup{
            "group.organic", {}, "Organic Overlay", true, 0.78,
            paperweight::CompositeMode::blend, {},
            paperweight::LayerMask{true, false, 719, 0.25, 0.82}, {}},
        paperweight::MaterialLayerGroup{
            "group.organic.detail", "group.organic", "Fine Detail", true, 0.64,
            paperweight::CompositeMode::detail, {}, {}, {}},
    };
    material.layerHierarchy = {
        {"layer.background", {}},
        {"layer.organic.base", "group.organic"},
        {"layer.organic.detail", "group.organic.detail"},
    };

    expect(!paperweight::validateMaterial(material),
           "nested layer groups with stable identities validate");
    const auto encoded = paperweight::serialisePmat(material);
    const auto* text = std::get_if<std::string>(&encoded);
    expect(text != nullptr && text->find("pmat.version = 24") != std::string::npos &&
               text->find("groups.count = 2") != std::string::npos &&
               text->find("group.1.parent = group.organic") != std::string::npos &&
               text->find("layer.2.parent = group.organic.detail") != std::string::npos,
           "PMAT 23 stores nested group hierarchy readably");
    if (text != nullptr) {
        const auto decoded = paperweight::parsePmat(*text);
        const auto* roundTripped = std::get_if<paperweight::Material>(&decoded);
        expect(roundTripped != nullptr && *roundTripped == material,
               "nested groups, masks, routes, names, and identities round-trip exactly");
    }

    for (const auto output : paperweight::materialOutputs) {
        const auto serial = paperweight::generate({
            material, 41, 37, output, std::nullopt, std::nullopt, 1});
        const auto parallel = paperweight::generate({
            material, 41, 37, output, std::nullopt, std::nullopt, 4});
        ...
        expect(serialImage != nullptr && parallelImage != nullptr &&
               ...
               "group compositing is byte-identical with one or four workers on every channel");
    }

    const auto oneTileResult = paperweight::generate({
        material, 20, 20, paperweight::MaterialOutput::height,
        std::nullopt, material.physicalSize, 1});
    const auto threeTilesResult = paperweight::generate({
        material, 60, 60, paperweight::MaterialOutput::height,
        std::nullopt, paperweight::PhysicalSize{3.0, 3.0}, 4});
    const auto* oneTile = std::get_if<paperweight::Image>(&oneTileResult);
    const auto* threeTiles = std::get_if<paperweight::Image>(&threeTilesResult);
    bool seamless = oneTile != nullptr && threeTiles != nullptr;
    for (std::uint32_t y = 0; seamless && y < 60; ++y) {
        for (std::uint32_t x = 0; x < 60; ++x) {
            if (threeTiles->row(y)[x] != oneTile->row(y % 20)[x % 20]) {
                seamless = false;
                break;
            }
        }
    }
    expect(seamless, "nested group masks remain byte-exact in 1x1 and 3x3 tiling");

    auto cycle = material;
    cycle.layerGroups.front().parentGroupIdentity = "group.organic.detail";
    expect(paperweight::validateMaterial(cycle).has_value(),
           "cyclic group ancestry is rejected");
    auto duplicate = material;
    duplicate.layerHierarchy.back().identity = "layer.background";
    expect(paperweight::validateMaterial(duplicate).has_value(),
           "duplicate layer and group identities are rejected");
    auto split = material;
    split.layerHierarchy.front().parentGroupIdentity = "group.organic";
    split.layerHierarchy[1].parentGroupIdentity.clear();
    expect(paperweight::validateMaterial(split).has_value(),
           "a group split into non-contiguous stack ranges is rejected");

    paperweight::LayerFragment groupedFragment{
        {material.layers[1], material.layers[2]},
        material.layerGroups,
        {material.layerHierarchy[1], material.layerHierarchy[2]},
    };
    const auto fragmentTextResult = paperweight::serialiseLayerFragment(groupedFragment);
    const auto* fragmentText = std::get_if<std::string>(&fragmentTextResult);
    expect(fragmentText != nullptr &&
               fragmentText->find("layer_fragment.version = 2") != std::string::npos,
           "grouped clipboard fragments use the backward-compatible version-2 envelope");
    if (fragmentText != nullptr) {
        const auto decoded = paperweight::parseLayerFragment(*fragmentText);
        expect(std::holds_alternative<paperweight::LayerFragment>(decoded) &&
                   std::get<paperweight::LayerFragment>(decoded) == groupedFragment,
               "grouped selections round-trip without flattening hierarchy");
    }

    const auto& overlays = paperweight::builtInMaterialOverlays();
    expect(overlays.size() >= 2 &&
               paperweight::findBuiltInMaterialOverlay("polished-moss") != nullptr &&
               paperweight::findBuiltInMaterialOverlay("polished-lichen") != nullptr,
           "polished moss and lichen ship as reusable seedless overlays");
    for (const auto& overlay : overlays) {
        const auto overlayTextResult = paperweight::serialiseMaterialOverlay(overlay);
        const auto* overlayText = std::get_if<std::string>(&overlayTextResult);
        expect(overlayText != nullptr &&
                   overlayText->find("# Paperweight reusable material overlay") == 0 &&
                   overlayText->find("groups.count = 1") != std::string::npos,
               "reusable overlays have a human-readable versioned source format");
        if (overlayText != nullptr) {
            const auto decoded = paperweight::parseMaterialOverlay(*overlayText);
            expect(std::holds_alternative<paperweight::MaterialOverlay>(decoded) &&
                       std::get<paperweight::MaterialOverlay>(decoded) == overlay,
                   "reusable overlay source round-trips exactly");
        }
    }
}
...
void testRegionAttributes()
{
    ...
    maskGraph.nodes = {
        ...
        paperweight::OutputNode{9, paperweight::MaterialOutput::metalness, 2},
        paperweight::OutputNode{10, paperweight::MaterialOutput::coating, 2},
        paperweight::OutputNode{11, paperweight::MaterialOutput::occlusion, 2},
        paperweight::OutputNode{12, paperweight::MaterialOutput::clearCoat, 2},
        paperweight::OutputNode{13, paperweight::MaterialOutput::clearCoatRoughness, 2},
        paperweight::OutputNode{14, paperweight::MaterialOutput::emissive, 2},
    };
    ...
    routedGraph.nodes = {
        ...
        paperweight::OutputNode{9, paperweight::MaterialOutput::metalness, 4},
        paperweight::OutputNode{10, paperweight::MaterialOutput::coating, 4},
        paperweight::OutputNode{11, paperweight::MaterialOutput::occlusion, 4},
        paperweight::OutputNode{12, paperweight::MaterialOutput::clearCoat, 4},
        paperweight::OutputNode{13, paperweight::MaterialOutput::clearCoatRoughness, 4},
        paperweight::OutputNode{14, paperweight::MaterialOutput::emissive, 2},
    };
    ...
    if (text != nullptr) {
        ...
        auto premature = *text;
        premature = downgradeCurrentPmat(std::move(premature), 8);
...
void testCourseLayouts()
{
    ...
    if (text != nullptr) {
        ...
        auto premature = *text;
        premature = downgradeCurrentPmat(std::move(premature), 9);
...
void testRegionSurfaceSculpting()
{
    ...
    if (text != nullptr) {
        ...
        auto premature = *text;
        premature = downgradeCurrentPmat(std::move(premature), 10);
        const auto rejected = paperweight::parsePmat(premature);
        expect(std::holds_alternative<paperweight::ParseDiagnostic>(rejected) &&
                   std::get<paperweight::ParseDiagnostic>(rejected).message.find(
...
void testShapePrimitivesAndLattices()
{
    ...
    if (text != nullptr) {
        ...
        auto versionEighteen = downgradeCurrentPmat(*text, 18);
...
void testDeterministicScatter()
{
    ...
    expect(scatterText != nullptr &&
               scatterText->find("pmat.version = 24") != std::string::npos &&
    ...
    if (scatterText != nullptr) {
        ...
        premature = downgradeCurrentPmat(std::move(premature), 12);
        const auto rejected = paperweight::parsePmat(premature);
...
void testOrganicStructures()
{
    ...
    expect(organicText != nullptr &&
               organicText->find("pmat.version = 24") != std::string::npos &&
    ...
    if (organicText != nullptr) {
        ...
        premature = downgradeCurrentPmat(std::move(premature), 13);
        const auto rejected = paperweight::parsePmat(premature);
        const auto* diagnostic = std::get_if<paperweight::ParseDiagnostic>(&rejected);
...
void testAdvancedSurfaceOperations()
{
    ...
    if (text != nullptr) {
        ...
        auto premature = *text;
        const auto marker = premature.find("pmat.version = 24");
...
void testMaterialGraph()
{
    ...
    expect(categoryCounts[static_cast<std::size_t>(
               ...
               categoryCounts[static_cast<std::size_t>(
                   paperweight::GraphNodeCategory::output)] == paperweight::materialOutputs.size(),
    ...
    routedGraph.nodes = {
        ...
        paperweight::OutputNode{64, paperweight::MaterialOutput::metalness, 50},
        paperweight::OutputNode{65, paperweight::MaterialOutput::coating, 50},
        paperweight::OutputNode{66, paperweight::MaterialOutput::occlusion, 50},
        paperweight::OutputNode{67, paperweight::MaterialOutput::clearCoat, 50},
        paperweight::OutputNode{68, paperweight::MaterialOutput::clearCoatRoughness, 50},
        paperweight::OutputNode{69, paperweight::MaterialOutput::emissive, 10},
    };
    ...
    cyclicGraph.nodes = {
        ...
        paperweight::OutputNode{8, paperweight::MaterialOutput::metalness, 2},
        paperweight::OutputNode{9, paperweight::MaterialOutput::coating, 2},
        paperweight::OutputNode{10, paperweight::MaterialOutput::occlusion, 2},
        paperweight::OutputNode{11, paperweight::MaterialOutput::clearCoat, 2},
        paperweight::OutputNode{12, paperweight::MaterialOutput::clearCoatRoughness, 2},
        paperweight::OutputNode{13, paperweight::MaterialOutput::emissive, 2},
    };
    expectGraphError(
        cyclicGraph,
...
void testReferenceTemplatesAndStylisedLighting()
{
    ...
    for (std::size_t templateIndex = 0; templateIndex < templates.size(); ++templateIndex) {
        ...
        std::array<std::optional<paperweight::Image>, paperweight::materialOutputs.size()> outputs;
...
void testPmat()
{
    ...
    const std::array showcaseGoldens{
        ...
        ShowcaseGolden{
            "woven-upholstery.pmat",
            {11855165821438660058ULL, 4017362141974645490ULL,
             12358976299149097270ULL, 2650504557225409266ULL}},
        ShowcaseGolden{
            "alternating-carpet.pmat",
            {14785652829194717461ULL, 12118325290182222723ULL,
             5179415732358359939ULL, 13018653927931753347ULL}},
        ShowcaseGolden{
            "heraldic-banner-cloth.pmat",
            {9006135550421082957ULL, 14630579878054693309ULL,
             15396739948278339414ULL, 16059683881342001910ULL}},
        ShowcaseGolden{
            "attached-paving.pmat",
            {17356369355658264478ULL, 15494849738318642065ULL,
             9371956096011566456ULL, 18161158170232902475ULL}},
        ShowcaseGolden{
            "arch-stone-panel.pmat",
            {8983970020371048834ULL, 12516620167815826164ULL,
             14004039655668764306ULL, 467841698457859920ULL}},
        ShowcaseGolden{
            "damaged-crate.pmat",
            {17589296438037884109ULL, 10845048536530470770ULL,
             9318266578987640354ULL, 5677434841520415106ULL}},
        ShowcaseGolden{
            "detailed-target-panel.pmat",
            {9917969139753378835ULL, 7145072805416155333ULL,
             17235131080766790117ULL, 14657877972709916189ULL}},
        ShowcaseGolden{
            "hierarchical-foliage.pmat",
            {1463068211007462449ULL, 16154294885953739900ULL,
             13768545888694199741ULL, 17171345092373346761ULL}},
        ShowcaseGolden{
            "hierarchical-ground.pmat",
            {7230883286268868448ULL, 17811815590268113870ULL,
             2848591958243391635ULL, 10558597383981011864ULL}},
        ShowcaseGolden{
            "rich-moss-colonies.pmat",
            {8896077823145676603ULL, 241124072455039396ULL,
             14947487786136963937ULL, 11482871670821357111ULL}},
        ShowcaseGolden{
            "rich-lichen-stamps.pmat",
            {11123900096935181452ULL, 10377649984556058064ULL,
             8744386877479885553ULL, 6811144031417867243ULL}},
    };
    constexpr std::array outputs{
        paperweight::MaterialOutput::colour,
        paperweight::MaterialOutput::height,
    ...
    if (brickMaterial != nullptr) {
        ...
        const auto versionMarkerPosition = versionFourBrick.find("pmat.version = 24");
        ...
        if (versionMarkerPosition != std::string::npos) {
            versionFourBrick = downgradeCurrentPmat(std::move(versionFourBrick), 4);
        }
    ...
    expectDiagnostic("pmat.version = 25\n", 1, "unsupported");
...
void testSurfaceChannelAuthoring()
{
    paperweight::Material material;
    material.roughnessLow = 0.0;
    material.roughnessHigh = 1.0;
    auto colour = paperweight::makeSolidColourLayer({200, 80, 40, 255});
    colour.outputs = {true, false, false, false, false, false, false, false, false};
    auto height = paperweight::makeSurfaceValueLayer(0.75);
    height.outputs = {false, true, false, false, false, false, false, false, false};
    auto roughness = paperweight::makeSurfaceValueLayer(0.25);
    roughness.outputs = {false, false, true, false, false, false, false, false, false};
    material.layers = {colour, height, roughness};

    const auto colourResult = paperweight::generate({
        material, 8, 8, paperweight::MaterialOutput::colour,
        std::nullopt, std::nullopt, 1});
    const auto heightResult = paperweight::generate({
        material, 8, 8, paperweight::MaterialOutput::height,
        std::nullopt, std::nullopt, 1});
    const auto roughnessResult = paperweight::generate({
        material, 8, 8, paperweight::MaterialOutput::roughness,
        std::nullopt, std::nullopt, 1});
    const auto* colourImage = std::get_if<paperweight::Image>(&colourResult);
    const auto* heightImage = std::get_if<paperweight::Image>(&heightResult);
    const auto* roughnessImage = std::get_if<paperweight::Image>(&roughnessResult);
    expect(colourImage != nullptr && colourImage->pixels().front() ==
               paperweight::Rgba8{200, 80, 40, 255},
           "a colour-only layer leaves an exact authored colour");
    expect(heightImage != nullptr &&
               heightImage->pixels().front().red > 185 &&
               heightImage->pixels().front().red < 195,
           "a height-only layer independently authors the height channel");
    expect(roughnessImage != nullptr &&
               roughnessImage->pixels().front().red > 60 &&
               roughnessImage->pixels().front().red < 68,
           "a roughness-only layer independently authors the roughness channel");

    const auto colourChecksum = colourImage != nullptr
        ? checksum(colourImage->pixels())
        : 0;
    const auto roughnessChecksum = roughnessImage != nullptr
        ? checksum(roughnessImage->pixels())
        : 0;
    std::get<paperweight::SurfaceValueOperation>(material.layers[1].operation).value = 0.15;
    const auto changedColour = paperweight::generate({
        material, 8, 8, paperweight::MaterialOutput::colour,
        std::nullopt, std::nullopt, 4});
    const auto changedRoughness = paperweight::generate({
        material, 8, 8, paperweight::MaterialOutput::roughness,
        std::nullopt, std::nullopt, 4});
    const auto* changedColourImage = std::get_if<paperweight::Image>(&changedColour);
    const auto* changedRoughnessImage = std::get_if<paperweight::Image>(&changedRoughness);
    expect(changedColourImage != nullptr && changedRoughnessImage != nullptr &&
               checksum(changedColourImage->pixels()) == colourChecksum &&
               checksum(changedRoughnessImage->pixels()) == roughnessChecksum,
           "editing one routed channel cannot perturb either untouched output");

    const paperweight::EvaluatedSample background{0.4, 0.4, 0.4, 0.4, 1.0, {}};
    const paperweight::EvaluatedSample low{0.2, 0.2, 0.2, 0.2, 1.0, {}};
    const paperweight::EvaluatedSample high{0.8, 0.8, 0.8, 0.8, 1.0, {}};
    expectNear(paperweight::compositeSamples(
                   background, low, paperweight::CompositeMode::minimum, 1.0).scalar,
               0.2, 0.0, "minimum composition retains the lower surface value");
    expectNear(paperweight::compositeSamples(
                   background, high, paperweight::CompositeMode::maximum, 1.0).scalar,
               0.8, 0.0, "maximum composition retains the higher surface value");
    expectNear(paperweight::compositeSamples(
                   background, high, paperweight::CompositeMode::detail, 0.5).scalar,
               0.7, 1.0e-12, "detail composition adds a centred fine-detail signal");

    paperweight::Material physical;
    physical.physicalSize = {1.0, 1.0};
    physical.reliefDepthMetres = 0.003;
    physical.normalStrength = 1.0;
    physical.layers = {paperweight::makeNoiseLayer()};
    auto normalEnergy = [](const paperweight::Image& image) {
        double total = 0.0;
        for (const auto& pixel : image.pixels()) {
            total += std::abs(static_cast<int>(pixel.red) - 128) +
                std::abs(static_cast<int>(pixel.green) - 128);
        }
        return total / static_cast<double>(image.pixels().size());
    };
    const auto normal64Result = paperweight::generate({
        physical, 64, 64, paperweight::MaterialOutput::normal,
        std::nullopt, std::nullopt, 1});
    const auto normal1024Result = paperweight::generate({
        physical, 1024, 1024, paperweight::MaterialOutput::normal,
        std::nullopt, std::nullopt, 4});
    const auto* normal64 = std::get_if<paperweight::Image>(&normal64Result);
    const auto* normal1024 = std::get_if<paperweight::Image>(&normal1024Result);
    if (normal64 != nullptr && normal1024 != nullptr) {
        const double lowResolutionEnergy = normalEnergy(*normal64);
        const double highResolutionEnergy = normalEnergy(*normal1024);
        expect(lowResolutionEnergy > 0.0 &&
                   std::abs(lowResolutionEnergy - highResolutionEnergy) /
                       highResolutionEnergy < 0.18,
               "physical relief produces resolution-independent normal-map strength");
    } else {
        expect(false, "physical normal-map fixtures generate at both resolutions");
    }
    physical.reliefDepthMetres = 0.0;
    const auto flatResult = paperweight::generate({
        physical, 32, 32, paperweight::MaterialOutput::normal,
        std::nullopt, std::nullopt, 2});
    const auto* flat = std::get_if<paperweight::Image>(&flatResult);
    expect(flat != nullptr && std::all_of(
               flat->pixels().begin(), flat->pixels().end(), [](const auto& pixel) {
                   return pixel.red == 128 && pixel.green == 128 && pixel.blue == 255;
               }),
           "zero physical relief generates a mathematically flat normal map");

    material.reliefDepthMetres = 0.003;
    ...
    expect(text != nullptr && text->find("pmat.version = 24") != std::string::npos &&
               ...
               text->find("composite = maximum") != std::string::npos,
           "PMAT 17 writes physical relief, output routes, values, and surface composites readably");
    if (text != nullptr) {
        const auto reparsed = paperweight::parsePmat(*text);
        expect(std::holds_alternative<paperweight::Material>(reparsed) &&
                   std::get<paperweight::Material>(reparsed) == material,
               "PMAT 17 surface authoring fields round-trip exactly");

        auto premature = *text;
        premature = downgradeCurrentPmat(std::move(premature), 15);
        const auto relief = premature.find("surface.relief_depth = 0.003m\n");
        premature.erase(relief, std::string("surface.relief_depth = 0.003m\n").size());
        const auto prematureResult = paperweight::parsePmat(premature);
        expect(std::holds_alternative<paperweight::ParseDiagnostic>(prematureResult) &&
                   std::get<paperweight::ParseDiagnostic>(prematureResult).message.find(
                       "require .pmat version 16") != std::string::npos,
               "older PMAT versions reject non-default surface channel routing explicitly");
    }

    auto invalid = material;
    invalid.layers.front().outputs = {
        false, false, false, false, false, false, false, false, false};
    expect(paperweight::validateMaterial(invalid).has_value(),
           "a layer with no output route is rejected explicitly");

    constexpr std::array surfaceShowcases{
        "polished-marble.pmat",
        "wet-mortar.pmat",
        "engraved-metal.pmat",
        "varnished-wood.pmat",
    };
    for (const auto* path : surfaceShowcases) {
        std::ifstream file(path, std::ios::binary);
        const std::string source(
            std::istreambuf_iterator<char>{file},
            std::istreambuf_iterator<char>{});
        const auto parsed = paperweight::parsePmat(source);
        const auto* showcase = std::get_if<paperweight::Material>(&parsed);
        expect(showcase != nullptr,
               "a v0.0.23 surface-channel showcase parses");
        if (showcase == nullptr) {
            continue;
        }
        const auto compiled = paperweight::compileMaterialGraph(*showcase);
        const auto* graph = std::get_if<paperweight::MaterialGraph>(&compiled);
        expect(graph != nullptr,
               "a v0.0.23 surface-channel showcase compiles to a graph");
        if (graph == nullptr) {
            continue;
        }
        for (const auto output : paperweight::materialOutputs) {
            const auto sample = paperweight::evaluateMaterialGraphSample(
                *showcase, *graph, output, 0.23, 0.61);
            const auto repeated = paperweight::evaluateMaterialGraphSample(
                *showcase, *graph, output, 1.23, -0.39);
            expect(sample.scalar == repeated.scalar &&
                       sample.red == repeated.red &&
                       sample.green == repeated.green &&
                       sample.blue == repeated.blue,
                   "every v0.0.23 showcase remains mathematically seamless per output");
        }
    }
}

void testMetalnessAndOptics()
{
    expectNear(paperweight::dielectricF0FromIor(1.0), 0.0, 0.0,
               "an IOR of one has no dielectric surface reflection");
    expectNear(paperweight::dielectricF0FromIor(1.5), 0.04, 1.0e-12,
               "the default dielectric IOR produces four-percent F0");

    paperweight::Material material;
    material.metalnessLow = 0.0;
    material.metalnessHigh = 1.0;
    material.dielectricIor = 1.52;
    auto colour = paperweight::makeSolidColourLayer({124, 86, 42, 255});
    colour.outputs = {true, false, false, false, false, false, false, false, false};
    auto metalness = paperweight::makeSurfaceValueLayer(0.75);
    metalness.outputs = {false, false, false, true, false, false, false, false, false};
    material.layers = {colour, metalness};

    paperweight::GenerationRequest request{
        material, 31, 23, paperweight::MaterialOutput::metalness,
        std::nullopt, std::nullopt, 1};
    const auto serial = paperweight::generate(request);
    request.workerCount = 4;
    const auto parallel = paperweight::generate(request);
    const auto* serialImage = std::get_if<paperweight::Image>(&serial);
    const auto* parallelImage = std::get_if<paperweight::Image>(&parallel);
    expect(serialImage != nullptr && serialImage->pixels().front().red >= 190 &&
               serialImage->pixels().front().red <= 192,
           "a metalness-only Surface Value layer generates deterministic UNORM8 metalness");
    expect(serialImage != nullptr && parallelImage != nullptr &&
               std::equal(serialImage->pixels().begin(), serialImage->pixels().end(),
                          parallelImage->pixels().begin()),
           "metalness is byte-identical with one or four workers");

    const auto colourBefore = paperweight::generate({
        material, 31, 23, paperweight::MaterialOutput::colour,
        std::nullopt, std::nullopt, 1});
    auto changedOptics = material;
    changedOptics.dielectricIor = 2.4;
    changedOptics.metalnessLow = 0.2;
    changedOptics.metalnessHigh = 0.9;
    const auto colourAfter = paperweight::generate({
        changedOptics, 31, 23, paperweight::MaterialOutput::colour,
        std::nullopt, std::nullopt, 4});
    const auto* beforeImage = std::get_if<paperweight::Image>(&colourBefore);
    const auto* afterImage = std::get_if<paperweight::Image>(&colourAfter);
    expect(beforeImage != nullptr && afterImage != nullptr &&
               checksum(beforeImage->pixels()) == checksum(afterImage->pixels()),
           "metalness and IOR never reinterpret or perturb authored colour-map bytes");
    ...
    expect(text != nullptr && text->find("pmat.version = 24") != std::string::npos &&
               ...
               text->find("outputs = metalness") != std::string::npos,
           "PMAT 17 stores metalness routing and dielectric optics readably");
    if (text != nullptr) {
        const auto reparsed = paperweight::parsePmat(*text);
        expect(std::holds_alternative<paperweight::Material>(reparsed) &&
                   std::get<paperweight::Material>(reparsed) == material,
        ...
        const auto marker = falselyVersioned.find("pmat.version = 24");
        falselyVersioned.replace(
            marker,
            std::string("pmat.version = 24").size(),
            "pmat.version = 16");
        const auto rejected = paperweight::parsePmat(falselyVersioned);
        expect(std::holds_alternative<paperweight::ParseDiagnostic>(rejected) &&
                   std::get<paperweight::ParseDiagnostic>(rejected).message.find(
                       "require .pmat version 17") != std::string::npos,
               "PMAT 16 rejects version-17-only metalness and optics fields");
    }
    ...
    constexpr std::string_view legacy =
        ...
        "roughness.high = 0.85\n"
        "layers.count = 0\n";
    const auto migrated = paperweight::parsePmat(legacy);
    const auto* legacyMaterial = std::get_if<paperweight::Material>(&migrated);
    expect(legacyMaterial != nullptr && legacyMaterial->metalnessLow == 0.0 &&
               legacyMaterial->metalnessHigh == 0.0 &&
               legacyMaterial->dielectricIor == 1.5,
           "PMAT 16 migrates to an explicit ordinary-dielectric optical default");

    auto invalid = material;
    invalid.dielectricIor = 0.9;
    expect(paperweight::validateMaterial(invalid).has_value(),
           "out-of-range dielectric IOR is rejected");
    invalid = material;
    invalid.metalnessHigh = 1.1;
    expect(paperweight::validateMaterial(invalid).has_value(),
           "out-of-range metalness is rejected");
}

void testMetalTemplates()
{
    constexpr std::array templateFiles{
        "chrome.pmat",
        "steel.pmat",
        "copper.pmat",
        "brass.pmat",
    ...
    for (std::size_t templateIndex = 0; templateIndex < templateFiles.size(); ++templateIndex) {
        std::ifstream file(templateFiles[templateIndex], std::ios::binary);
        const std::string source(
            std::istreambuf_iterator<char>{file},
            std::istreambuf_iterator<char>{});
        const auto parsed = paperweight::parsePmat(source);
        const auto* material = std::get_if<paperweight::Material>(&parsed);
        expect(material != nullptr && material->metadata.has_value() &&
                   material->metadata->category == "Metal" &&
                   material->metalnessHigh > 0.9,
               "each v0.0.24 metal template has identity and optical material state");
        if (material == nullptr) {
            continue;
        }

        const auto canonical = paperweight::serialisePmat(*material);
        const auto* canonicalText = std::get_if<std::string>(&canonical);
        bool roundTrips = false;
        if (canonicalText != nullptr) {
            const auto roundTrip = paperweight::parsePmat(*canonicalText);
            roundTrips = std::holds_alternative<paperweight::Material>(roundTrip) &&
                std::get<paperweight::Material>(roundTrip) == *material;
        }
        expect(roundTrips,
               "v0.0.24 metal templates round-trip through PMAT 17 exactly");

        for (const auto output : paperweight::materialOutputs) {
            paperweight::GenerationRequest request{
                *material, 20, 20, output, std::nullopt, material->physicalSize, 1};
            const auto serial = paperweight::generate(request);
            request.workerCount = 4;
            const auto parallel = paperweight::generate(request);
            const auto* oneTile = std::get_if<paperweight::Image>(&serial);
            const auto* parallelTile = std::get_if<paperweight::Image>(&parallel);
            expect(oneTile != nullptr && parallelTile != nullptr &&
                       std::equal(oneTile->pixels().begin(), oneTile->pixels().end(),
                                  parallelTile->pixels().begin()),
                   "metal-template maps are byte-identical across worker counts");
            if (oneTile == nullptr) {
                continue;
            }
            ...
            request.width = 60;
            request.height = 60;
            request.physicalCoverage = paperweight::PhysicalSize{
                material->physicalSize.widthMetres * 3.0,
                material->physicalSize.heightMetres * 3.0,
            };
            const auto triple = paperweight::generate(request);
            const auto* tripleTile = std::get_if<paperweight::Image>(&triple);
            bool repeatsExactly = tripleTile != nullptr;
            for (std::uint32_t tileY = 0; tileY < 3 && repeatsExactly; ++tileY) {
                for (std::uint32_t tileX = 0; tileX < 3 && repeatsExactly; ++tileX) {
                    for (std::uint32_t y = 0; y < 20 && repeatsExactly; ++y) {
                        for (std::uint32_t x = 0; x < 20; ++x) {
                            if (oneTile->row(y)[x] !=
                                tripleTile->row(tileY * 20 + y)[tileX * 20 + x]) {
                                repeatsExactly = false;
                                break;
                            }
                        }
            ...
            expect(repeatsExactly,
                   "every metal-template output repeats exactly in a 3 by 3 coverage");
        }

        const auto metalness = paperweight::generate({
            *material, 32, 32, paperweight::MaterialOutput::metalness,
            std::nullopt, material->physicalSize, 1});
        const auto* metalnessImage = std::get_if<paperweight::Image>(&metalness);
        if (metalnessImage != nullptr && templateIndex < 4) {
            expect(std::all_of(
                       metalnessImage->pixels().begin(), metalnessImage->pixels().end(),
                       [](paperweight::Rgba8 pixel) { return pixel.red == 255; }),
                   "clean elemental metal templates remain fully metallic");
        } else if (metalnessImage != nullptr) {
            const auto [minimum, maximum] = std::minmax_element(
                metalnessImage->pixels().begin(), metalnessImage->pixels().end(),
                [](paperweight::Rgba8 left, paperweight::Rgba8 right) {
                    return left.red < right.red;
                });
            expect(minimum->red + 16 < maximum->red,
                   "painted and corroded templates contain meaningful metalness variation");
        }
    }
}

void testCoatingsAndSpecialSurfaces()
{
    auto material = paperweight::Material{};
    material.seed = 250025;
    material.layers = {paperweight::makeNoiseLayer(91)};
    material.coatingLow = 1.0;
    material.coatingHigh = 0.0;
    material.occlusionLow = 0.3;
    material.occlusionHigh = 1.0;
    material.clearCoatLow = 0.15;
    material.clearCoatHigh = 0.95;
    material.clearCoatRoughnessLow = 0.32;
    material.clearCoatRoughnessHigh = 0.04;
    material.emissiveIntensity = 0.8;
    material.anisotropyStrength = 0.75;
    material.anisotropyRotationDegrees = 90.0;

    expect(!paperweight::validateMaterial(material),
           "coating, AO, clear-coat, emissive, and anisotropy values validate");
    const auto serialised = paperweight::serialisePmat(material);
    const auto* text = std::get_if<std::string>(&serialised);
    const bool storesSpecialSurfaceState =
        text != nullptr && text->find("pmat.version = 24") != std::string::npos &&
               ...
               text->find("emissive.intensity = 0.8") != std::string::npos &&
               text->find("anisotropy.rotation = ") != std::string::npos &&
               text->find("clearcoat_roughness") != std::string::npos;
    expect(storesSpecialSurfaceState,
           "PMAT 18 stores special-surface state and routing readably");
    if (text != nullptr) {
        const auto parsed = paperweight::parsePmat(*text);
        expect(std::holds_alternative<paperweight::Material>(parsed) &&
                   std::get<paperweight::Material>(parsed) == material,
               "PMAT 18 special-surface state round-trips exactly");
    }

    for (const auto output : std::array{
             paperweight::MaterialOutput::coating,
             paperweight::MaterialOutput::occlusion,
             paperweight::MaterialOutput::clearCoat,
             paperweight::MaterialOutput::clearCoatRoughness,
             paperweight::MaterialOutput::emissive,
         }) {
        auto request = paperweight::GenerationRequest{
            material, 37, 29, output, std::nullopt, std::nullopt, 1};
        const auto serial = paperweight::generate(request);
        request.workerCount = 4;
        const auto parallel = paperweight::generate(request);
        const auto* serialImage = std::get_if<paperweight::Image>(&serial);
        ...
        expect(serialImage != nullptr && parallelImage != nullptr &&
                   std::equal(serialImage->pixels().begin(), serialImage->pixels().end(),
                              parallelImage->pixels().begin()),
               "special-surface maps are byte-identical across worker counts");
    }
    ...
    constexpr std::string_view legacy =
        ...
        "surface.ior = 1.5\n"
        "layers.count = 0\n";
    const auto legacyParsed = paperweight::parsePmat(legacy);
    const auto* migrated = std::get_if<paperweight::Material>(&legacyParsed);
    expect(migrated != nullptr && migrated->coatingLow == 0.0 &&
               migrated->coatingHigh == 0.0 && migrated->occlusionLow == 1.0 &&
               migrated->occlusionHigh == 1.0 && migrated->clearCoatLow == 0.0 &&
               migrated->emissiveIntensity == 0.0 && migrated->anisotropyStrength == 0.0,
           "PMAT 17 migrates with visually neutral v18 channel defaults");
    if (migrated != nullptr) {
        for (const auto output : std::array{
                 paperweight::MaterialOutput::colour,
                 paperweight::MaterialOutput::height,
                 paperweight::MaterialOutput::normal,
                 paperweight::MaterialOutput::roughness,
                 paperweight::MaterialOutput::metalness,
             }) {
            const auto before = paperweight::generate({
                paperweight::Material{}, 31, 23, output, std::nullopt, std::nullopt, 1});
            const auto after = paperweight::generate({
                *migrated, 31, 23, output, std::nullopt, std::nullopt, 4});
            const auto* beforeImage = std::get_if<paperweight::Image>(&before);
            const auto* afterImage = std::get_if<paperweight::Image>(&after);
            expect(beforeImage != nullptr && afterImage != nullptr &&
                       checksum(beforeImage->pixels()) == checksum(afterImage->pixels()),
                   "PMAT 17 migration preserves every historical output byte");
        }
    }
    const auto premature = paperweight::parsePmat(
        std::string(legacy) + "clearcoat.low = 0.5\n");
    expect(std::holds_alternative<paperweight::ParseDiagnostic>(premature) &&
               std::get<paperweight::ParseDiagnostic>(premature).message.find(
                   "version 18") != std::string::npos,
           "PMAT 17 rejects v18-only special-surface fields explicitly");

    for (const std::string_view name : {
             "glazed-ceramic", "lacquered-wood", "wet-stone",
             "machinery-panels", "illuminated-scifi",
         }) {
        ...
        const auto parsed = paperweight::parsePmat(source);
        const auto* authored = std::get_if<paperweight::Material>(&parsed);
        expect(authored != nullptr && authored->metadata &&
                   paperweight::findWizardMaterialTemplate(name) != nullptr,
               "each v0.0.25 template has identity and wizard metadata");
        if (authored == nullptr) {
            continue;
        }
        for (const auto output : paperweight::materialOutputs) {
            auto request = paperweight::GenerationRequest{
                *authored, 19, 17, output, std::nullopt, std::nullopt, 1};
            const auto serial = paperweight::generate(request);
            request.workerCount = 4;
            const auto parallel = paperweight::generate(request);
            const auto* serialImage = std::get_if<paperweight::Image>(&serial);
            ...
            expect(serialImage != nullptr && parallelImage != nullptr &&
                       std::equal(serialImage->pixels().begin(), serialImage->pixels().end(),
                                  parallelImage->pixels().begin()),
                   "v0.0.25 template outputs are deterministic across worker counts");
        }
    }
}
...
void testMultiOutputGeneration()
{
    ...
    before = paperweight::Material{};
    before.layers = {paperweight::makeNoiseLayer()};
    before.layerGroups = {paperweight::MaterialLayerGroup{
        "group.surface", {}, "Surface", true, 1.0,
        paperweight::CompositeMode::blend, {}, {},
        {false, false, true, false, false, false, false, false, false}}};
    before.layerHierarchy = {{"layer.surface", "group.surface"}};
    after = before;
    after.layerGroups.front().name = "Renamed Surface";
    const auto renamedGroup = paperweight::affectedMaterialOutputs(before, after);
    expect(std::none_of(renamedGroup.begin(), renamedGroup.end(), [](bool value) { return value; }),
           "renaming a group does not regenerate deterministic maps");
    after.layerGroups.front().opacity = 0.4;
    const auto roughnessGroup = paperweight::affectedMaterialOutputs(before, after);
    expect(selected(roughnessGroup, paperweight::MaterialOutput::roughness) &&
               std::count(roughnessGroup.begin(), roughnessGroup.end(), true) == 1,
           "a routed group edit invalidates only its affected output");
}
...
void testMaterialWizard()
{
    ...
    expect(metalTemplates.size() == 10 &&
               metalTemplates[0]->identifier == "painted-metal" &&
               metalTemplates[1]->identifier == "weathered-metal" &&
               metalTemplates[2]->identifier == "engraved-metal" &&
               metalTemplates[3]->identifier == "chrome" &&
               metalTemplates[8]->identifier == "corroded-metal",
           "metal offers legacy, optical, and special-surface starting points");
...
void testMaterialIdentityAndLibrary()
{
    ...
    if (text != nullptr) {
        ...
        oldVersion = downgradeCurrentPmat(std::move(oldVersion), 14);
...
void testPackedMaterialLibrary()
{
    ...
    if (library->checksum() != 7312223191231703899ULL) {
        std::cerr << "Expected packed-library checksum 7312223191231703899, got "
                  << library->checksum() << '\n';
    }
    expect(library->checksum() == 7312223191231703899ULL,
...
void testOrganicSilhouettesAndHierarchicalClusters()
{
    constexpr std::uint64_t seed = 300030;
    auto clusters = paperweight::LeafClusterOperation{};
    clusters.profile = paperweight::LeafProfile::blob;
    clusters.pattern = paperweight::LeafClusterPattern::groundScatter;
    clusters.columns = 4;
    clusters.rows = 4;
    clusters.leavesPerCluster = 20;
    clusters.density = 1.0;
    clusters.clusterSpread = 0.09;
    clusters.leafLength = 0.16;
    clusters.leafWidth = 0.14;
    clusters.lobing = 0.42;
    clusters.lobeCount = 7;
    clusters.edgeWidth = 0.12;
    clusters.innerHighlightWidth = 0.12;
    clusters.innerHighlightInset = 0.05;
    clusters.clusterColourVariation = 0.65;
    clusters.instanceColourVariation = 0.35;
    clusters.secondaryProfile = paperweight::LeafProfile::rosette;
    clusters.secondaryWeight = 0.34;
    clusters.secondaryScale = 0.62;
    clusters.tertiaryProfile = paperweight::LeafProfile::lichen;
    clusters.tertiaryWeight = 0.28;
    clusters.tertiaryScale = 0.38;
    clusters.seedOffset = 307;

    const auto layout = paperweight::buildLeafClusterLayout(clusters, seed);
    expect(layout == paperweight::buildLeafClusterLayout(clusters, seed),
           "hierarchical organic layout is reproducible from stable hashes");
    std::array<bool, 3> populations{};
    bool sharedClusterVariation = false;
    for (std::size_t index = 0; index < layout.instances.size(); ++index) {
        const auto& instance = layout.instances[index];
        populations[std::min<std::size_t>(instance.population, 2)] = true;
        for (std::size_t other = index + 1; other < layout.instances.size(); ++other) {
            if (layout.instances[other].clusterKey == instance.clusterKey) {
                sharedClusterVariation = sharedClusterVariation ||
                    layout.instances[other].clusterRandom == instance.clusterRandom;
                break;
            }
        }
    }
    expect(populations[0] && populations[1] && populations[2] && sharedClusterVariation,
           "one hierarchy contains three overlapping populations with shared cluster colour state");

    double maximumOutline = 0.0;
    double maximumHighlight = 0.0;
    double maximumFill = 0.0;
    for (std::uint32_t y = 0; y < 128; ++y) {
        for (std::uint32_t x = 0; x < 128; ++x) {
            const double u = (static_cast<double>(x) + 0.5) / 128.0;
            const double v = (static_cast<double>(y) + 0.5) / 128.0;
            const auto sample = paperweight::evaluateLeafCluster(clusters, layout, u, v);
            const auto wrapped = paperweight::evaluateLeafCluster(clusters, layout, u + 1.0, v - 1.0);
            expect(sample == wrapped,
                   "blobby and lobed organic silhouettes repeat exactly across the texture torus");
            maximumFill = std::max(maximumFill, sample.coverage);
            maximumOutline = std::max(maximumOutline, sample.outline);
            maximumHighlight = std::max(maximumHighlight, sample.innerHighlight);
        }
    }
    expect(maximumFill > 0.9 && maximumOutline > 0.2 && maximumHighlight > 0.2,
           "organic silhouettes expose useful fill, contour, and inner-highlight bands");

    auto growth = paperweight::OrganicAccumulationOperation{};
    growth.profile = paperweight::OrganicAccumulationProfile::colonies;
    growth.field = paperweight::OrganicAccumulationField::material;
    growth.outlineWidth = 0.09;
    growth.innerHighlightWidth = 0.11;
    growth.innerHighlightInset = 0.06;
    growth.seedOffset = 311;
    const paperweight::RegionSample region{91, 0.2, 0.7, 0.6, 0.1, true};
    const auto colony = paperweight::evaluateOrganicAccumulation(
        growth, 0.28, region, 0.25, 0.625, seed);
    const auto wrappedColony = paperweight::evaluateOrganicAccumulation(
        growth, 0.28, region, 1.25, -0.375, seed);
    expect(colony == wrappedColony && colony.fill >= 0.0 && colony.fill <= 1.0 &&
               colony.outline >= 0.0 && colony.outline <= 1.0 &&
               colony.innerHighlight >= 0.0 && colony.innerHighlight <= 1.0,
           "richer colony accumulation is seamless and every field stays bounded");

    paperweight::Material material;
    material.seed = seed;
    auto clusterLayer = paperweight::makeLeafClusterLayer();
    std::get<paperweight::LeafClusterOperation>(clusterLayer.operation) = clusters;
    auto growthLayer = paperweight::makeOrganicAccumulationLayer();
    std::get<paperweight::OrganicAccumulationOperation>(growthLayer.operation) = growth;
    material.layers = {clusterLayer, growthLayer};
    expect(!paperweight::validateMaterial(material),
           "v0.0.30 hierarchical organic material validates");

    const auto encoded = paperweight::serialisePmat(material);
    const auto* text = std::get_if<std::string>(&encoded);
    expect(text != nullptr && text->find("pmat.version = 24") != std::string::npos &&
               text->find("leaf.secondary_profile = rosette") != std::string::npos &&
               text->find("leaf.tertiary_profile = lichen") != std::string::npos &&
               text->find("organic.accumulation.profile = colonies") != std::string::npos &&
               text->find("organic.accumulation.inner_highlight_width") != std::string::npos,
           "PMAT version 22 writes the complete organic hierarchy readably");
    if (text != nullptr) {
        const auto decoded = paperweight::parsePmat(*text);
        const auto* roundTripped = std::get_if<paperweight::Material>(&decoded);
        expect(roundTripped != nullptr && *roundTripped == material,
               "PMAT version 22 round-trips organic populations and accumulation exactly");
        auto oldVersion = downgradeCurrentPmat(*text, 21);
        const auto rejected = paperweight::parsePmat(oldVersion);
        const auto* diagnostic = std::get_if<paperweight::ParseDiagnostic>(&rejected);
        if (diagnostic != nullptr && diagnostic->message.find("version 22") == std::string::npos) {
            std::cerr << "Expected explicit PMAT v22 diagnostic, got: "
                      << diagnostic->message << '\n';
        }
        expect(diagnostic != nullptr && diagnostic->message.find("version 22") != std::string::npos,
               "PMAT version 21 rejects v22 organic hierarchy fields explicitly");
    }

    std::ifstream legacyLeafFile("castle-foliage.pmat", std::ios::binary);
    std::string legacyLeafText(
        std::istreambuf_iterator<char>{legacyLeafFile},
        std::istreambuf_iterator<char>{});
    const auto legacyProfile = legacyLeafText.find("leaf.profile = cordate");
    if (legacyProfile != std::string::npos) {
        legacyLeafText.replace(
            legacyProfile,
            std::string_view("leaf.profile = cordate").size(),
            "leaf.profile = blob");
    }
    const auto enumSmuggle = paperweight::parsePmat(legacyLeafText);
    const auto* enumDiagnostic = std::get_if<paperweight::ParseDiagnostic>(&enumSmuggle);
    expect(enumDiagnostic != nullptr &&
               enumDiagnostic->message.find("version 22") != std::string::npos,
           "a v22-only organic silhouette cannot be smuggled into a legacy PMAT version");

    for (const auto output : paperweight::materialOutputs) {
        const auto serial = paperweight::generate({
            material, 47, 41, output, std::nullopt, std::nullopt, 1,
        });
        const auto parallel = paperweight::generate({
            material, 47, 41, output, std::nullopt, std::nullopt, 4,
        });
        const auto* serialImage = std::get_if<paperweight::Image>(&serial);
        const auto* parallelImage = std::get_if<paperweight::Image>(&parallel);
        expect(serialImage != nullptr && parallelImage != nullptr &&
                   std::equal(serialImage->pixels().begin(), serialImage->pixels().end(),
                              parallelImage->pixels().begin()),
               "v0.0.30 organic output is byte-identical with one and four workers");
    }

    auto invalid = material;
    auto& invalidClusters = std::get<paperweight::LeafClusterOperation>(
        invalid.layers.front().operation);
    invalidClusters.secondaryWeight = 0.8;
    invalidClusters.tertiaryWeight = 0.4;
    expect(paperweight::validateMaterial(invalid).has_value(),
           "organic population weights cannot exceed one combined");
}

void testTextilesAndFibres()
{
    paperweight::TextileOperation textile;
    textile.pattern = paperweight::TextilePattern::twillWeave;
    textile.field = paperweight::TextileField::material;
    textile.yarnProfile = paperweight::YarnProfile::twisted;
    textile.tileOrientation = paperweight::TextileTileOrientation::checkerboard;
    textile.columns = 18;
    textile.rows = 14;
    textile.tileColumns = 2;
    textile.tileRows = 2;
    textile.weaveSpan = 3;
    textile.twillStep = 2;
    textile.yarnWidth = 0.91;
    textile.yarnRoundness = 0.72;
    textile.crossingHeight = 0.24;
    textile.jitter = 0.16;
    textile.fibreFrequency = 17;
    textile.fibreStrength = 0.21;
    textile.twist = 0.43;
    textile.missingAmount = 0.04;
    textile.damageAmount = 0.08;
    textile.differentColourAmount = 0.07;
    textile.colourVariation = 0.35;
    textile.seedOffset = 2819;

    for (const auto pattern : {
             paperweight::TextilePattern::plainWeave,
             paperweight::TextilePattern::basketWeave,
             paperweight::TextilePattern::twillWeave,
             paperweight::TextilePattern::loopPile,
             paperweight::TextilePattern::cutPile,
         }) {
        textile.pattern = pattern;
        const auto sample = paperweight::evaluateTextile(textile, 0.173, 0.619, 280028);
        const auto repeatX = paperweight::evaluateTextile(textile, 1.173, 0.619, 280028);
        const auto repeatY = paperweight::evaluateTextile(textile, 0.173, -0.381, 280028);
        const auto sameSample = [&sample](const paperweight::TextileSample& repeated) {
            return std::abs(sample.coverage - repeated.coverage) <= 1.0e-12 &&
                std::abs(sample.height - repeated.height) <= 1.0e-12 &&
                std::abs(sample.fibres - repeated.fibres) <= 1.0e-12 &&
                std::abs(sample.damage - repeated.damage) <= 1.0e-12 &&
                sample.colour == repeated.colour && sample.region.key == repeated.region.key;
        };
        expect(sameSample(repeatX) && sameSample(repeatY),
               "every textile and pile pattern is mathematically periodic on the material torus");
        expect(sample.coverage >= 0.0 && sample.coverage <= 1.0 &&
                   sample.height >= 0.0 && sample.height <= 1.0 &&
                   sample.roughness >= 0.0 && sample.roughness <= 1.0,
               "textile material fields stay normalised");
    }

    textile.pattern = paperweight::TextilePattern::plainWeave;
    const auto woven = paperweight::evaluateTextile(textile, 0.21, 0.37, 280028);
    textile.pattern = paperweight::TextilePattern::loopPile;
    const auto loop = paperweight::evaluateTextile(textile, 0.21, 0.37, 280028);
    textile.pattern = paperweight::TextilePattern::cutPile;
    const auto tuft = paperweight::evaluateTextile(textile, 0.21, 0.37, 280028);
    expect(woven.pile == 0.0 && (loop.pile > 0.0 || tuft.pile > 0.0),
           "woven yarn and raised pile expose distinct reusable fields");

    paperweight::Material material;
    material.seed = 280028;
    material.metadata = paperweight::MaterialMetadata{
        "28002800-0000-4000-8000-000000000028",
        "Textile Round Trip",
        "A complete version-20 textile fixture.",
        "textile",
        {"textile", "test"},
    };
    textile.pattern = paperweight::TextilePattern::twillWeave;
    material.layers = {paperweight::makeTextileLayer()};
    std::get<paperweight::TextileOperation>(material.layers.front().operation) = textile;
    expect(!paperweight::validateMaterial(material),
           "a complete textile operation passes material validation");

    const auto encoded = paperweight::serialisePmat(material);
    const auto* text = std::get_if<std::string>(&encoded);
    expect(text != nullptr && text->find("pmat.version = 24") != std::string::npos &&
               text->find("textile.pattern = twill_weave") != std::string::npos &&
               text->find("textile.tile_orientation = checkerboard") != std::string::npos,
           "PMAT version 20 serialises the complete textile vocabulary readably");
    if (text != nullptr) {
        const auto decoded = paperweight::parsePmat(*text);
        const auto* roundTripped = std::get_if<paperweight::Material>(&decoded);
        expect(roundTripped != nullptr && *roundTripped == material,
               "PMAT version 20 round-trips textile operations exactly");
        auto oldVersion = *text;
        const auto versionPosition = oldVersion.find("pmat.version = 24");
        oldVersion.replace(versionPosition, std::string_view{"pmat.version = 24"}.size(),
                           "pmat.version = 19");
        expect(std::holds_alternative<paperweight::ParseDiagnostic>(
                   paperweight::parsePmat(oldVersion)),
               "textile fields are rejected when mislabeled as PMAT version 19");
    }

    const auto compiled = paperweight::compileMaterialGraph(material);
    const auto* graph = std::get_if<paperweight::MaterialGraph>(&compiled);
    expect(graph != nullptr && std::any_of(
               graph->nodes.begin(), graph->nodes.end(), [](const auto& node) {
                   const auto* generator = std::get_if<paperweight::GeneratorNode>(&node);
                   return generator != nullptr && std::holds_alternative<
                       paperweight::TextileOperation>(generator->operation);
               }),
           "textile layers compile into reusable material-graph generator nodes");

    std::array<std::uint64_t, paperweight::materialOutputs.size()> serialChecksums{};
    for (const auto output : paperweight::materialOutputs) {
        const auto serial = paperweight::generate({
            material, 37, 29, output, std::nullopt, std::nullopt, 1,
        });
        const auto parallel = paperweight::generate({
            material, 37, 29, output, std::nullopt, std::nullopt, 4,
        });
        const auto* serialImage = std::get_if<paperweight::Image>(&serial);
        const auto* parallelImage = std::get_if<paperweight::Image>(&parallel);
        const auto outputIndex = static_cast<std::size_t>(output);
        if (serialImage != nullptr) serialChecksums[outputIndex] = checksum(serialImage->pixels());
        expect(serialImage != nullptr && parallelImage != nullptr &&
                   serialImage->pixels().size() == parallelImage->pixels().size() &&
                   std::equal(serialImage->pixels().begin(), serialImage->pixels().end(),
                              parallelImage->pixels().begin()),
               "textile output is byte-identical with one and four workers");
    }
    const auto changedSeedMaterial = [&] {
        auto changed = material;
        ++changed.seed;
        return changed;
    }();
    const auto changed = paperweight::generate({
        changedSeedMaterial, 37, 29, paperweight::MaterialOutput::colour,
        std::nullopt, std::nullopt, 4,
    });
    const auto* changedImage = std::get_if<paperweight::Image>(&changed);
    expect(changedImage != nullptr &&
               checksum(changedImage->pixels()) !=
                   serialChecksums[static_cast<std::size_t>(paperweight::MaterialOutput::colour)],
           "changing a material seed changes stable textile variation and damage");

    auto invalid = material;
    auto& invalidTextile = std::get<paperweight::TextileOperation>(
        invalid.layers.front().operation);
    invalidTextile.tileRows = 3;
    expect(paperweight::validateMaterial(invalid).has_value(),
           "alternating textile tiles reject odd repeat counts that would break seams");
}

void testRegionAttachedDetailAndDamage()
{
    paperweight::CourseLayoutOperation layout;
    layout.profile = paperweight::CourseLayoutProfile::slabs;
    layout.blocks = 5;
    layout.courses = 4;
    layout.blockVariation = 0.42;
    layout.courseVariation = 0.24;
    layout.crookedness = 0.09;
    layout.gap = 0.055;
    layout.seedOffset = 2901;
    const paperweight::PhysicalSize physicalSize{2.0, 1.2};
    const auto region = paperweight::evaluateCourseLayoutFields(
        layout, physicalSize, 0.37, 0.41, 290029).region;
    const auto repeatedRegion = paperweight::evaluateCourseLayoutFields(
        layout, physicalSize, 1.37, -0.59, 290029).region;
    expect(region.valid && repeatedRegion.valid && region.key == repeatedRegion.key &&
               std::abs(region.localU - repeatedRegion.localU) <= 1.0e-12 &&
               std::abs(region.localV - repeatedRegion.localV) <= 1.0e-12,
           "structural regions expose stable tile-periodic local coordinates");

    paperweight::ShapePrimitiveOperation rotatedShape;
    rotatedShape.kind = paperweight::ShapePrimitiveKind::roundedRectangle;
    rotatedShape.rotationDegrees = 90.0;
    const auto orientedRegion = paperweight::evaluateShapePrimitive(
        rotatedShape, 0.5, 0.5).region;
    expectNear(orientedRegion.orientationTurns, 0.25, 1.0e-12,
               "rotated region producers expose their stable local-frame orientation");

    for (const auto anchor : {
             paperweight::RegionAnchor::centre,
             paperweight::RegionAnchor::edge,
             paperweight::RegionAnchor::corner,
             paperweight::RegionAnchor::cavity,
         }) {
        const auto first = paperweight::resolveRegionAnchor(
            region, anchor, 1, 3, 290029, 77, 0.14);
        const auto repeated = paperweight::resolveRegionAnchor(
            repeatedRegion, anchor, 1, 3, 290029, 77, 0.14);
        expect(first == repeated && first.u >= 0.0 && first.u <= 1.0 &&
                   first.v >= 0.0 && first.v <= 1.0,
               "centre, edge, corner, and cavity anchors are stable in region-local space");
    }

    paperweight::RegionAttachmentOperation attachment;
    attachment.kind = paperweight::RegionAttachmentKind::damage;
    attachment.field = paperweight::RegionAttachmentField::material;
    attachment.startAnchor = paperweight::RegionAnchor::corner;
    attachment.endAnchor = paperweight::RegionAnchor::edge;
    attachment.glyph = paperweight::RegionGlyph::rune;
    attachment.count = 3;
    attachment.size = 0.28;
    attachment.aspect = 1.35;
    attachment.inset = 0.1;
    attachment.rotationDegrees = 17.0;
    attachment.jitter = 0.22;
    attachment.selection = 0.84;
    attachment.lineWidth = 0.032;
    attachment.length = 0.78;
    attachment.branching = 0.31;
    attachment.softness = 0.012;
    attachment.colour = {52, 39, 31, 255};
    attachment.height = 0.16;
    attachment.roughness = 0.91;
    attachment.metalness = 0.05;
    attachment.occlusion = 0.22;
    attachment.emissive = 0.03;
    attachment.seedOffset = 2902;

    for (const auto kind : {
             paperweight::RegionAttachmentKind::fastener,
             paperweight::RegionAttachmentKind::inlay,
             paperweight::RegionAttachmentKind::glyph,
             paperweight::RegionAttachmentKind::chip,
             paperweight::RegionAttachmentKind::crack,
             paperweight::RegionAttachmentKind::damage,
         }) {
        attachment.kind = kind;
        const auto first = paperweight::evaluateRegionAttachment(
            attachment, region, 290029);
        const auto repeated = paperweight::evaluateRegionAttachment(
            attachment, repeatedRegion, 290029);
        expect(std::abs(first.coverage - repeated.coverage) <= 1.0e-12 &&
                   std::abs(first.signedDistance - repeated.signedDistance) <= 1.0e-12 &&
                   first.region.key == repeated.region.key,
               "every attached detail profile repeats exactly with its source region");
    }

    paperweight::Material material;
    material.seed = 290029;
    material.physicalSize = physicalSize;
    material.metadata = paperweight::MaterialMetadata{
        "29002900-0000-4000-8000-000000000029",
        "Region Attachment Round Trip",
        "A complete version-21 region-attached damage fixture.",
        "constructed",
        {"attachment", "damage", "test"},
    };
    auto base = paperweight::makeCourseLayoutLayer();
    std::get<paperweight::CourseLayoutOperation>(base.operation) = layout;
    auto detail = paperweight::makeRegionAttachmentLayer();
    attachment.kind = paperweight::RegionAttachmentKind::damage;
    std::get<paperweight::RegionAttachmentOperation>(detail.operation) = attachment;
    material.layers = {base, detail};
    expect(!paperweight::validateMaterial(material),
           "a complete region attachment passes material validation");

    const auto encoded = paperweight::serialisePmat(material);
    const auto* text = std::get_if<std::string>(&encoded);
    expect(text != nullptr && text->find("pmat.version = 24") != std::string::npos &&
               text->find("operation = region_attachment") != std::string::npos &&
               text->find("attachment.start_anchor = corner") != std::string::npos &&
               text->find("attachment.kind = damage") != std::string::npos,
           "PMAT version 21 serialises the complete region-attachment vocabulary readably");
    if (text != nullptr) {
        const auto decoded = paperweight::parsePmat(*text);
        const auto* roundTripped = std::get_if<paperweight::Material>(&decoded);
        expect(roundTripped != nullptr && *roundTripped == material,
               "PMAT version 21 round-trips region attachments exactly");
        auto oldVersion = *text;
        const auto versionPosition = oldVersion.find("pmat.version = 24");
        oldVersion.replace(versionPosition, std::string_view{"pmat.version = 24"}.size(),
                           "pmat.version = 20");
        expect(std::holds_alternative<paperweight::ParseDiagnostic>(
                   paperweight::parsePmat(oldVersion)),
               "region attachment fields are rejected when mislabeled as PMAT version 20");
    }
    ...
    expect(graph != nullptr && std::any_of(
               graph->nodes.begin(), graph->nodes.end(), [](const auto& node) {
                   const auto* processing = std::get_if<paperweight::ProcessingNode>(&node);
                   return processing != nullptr && std::holds_alternative<
                       paperweight::RegionAttachmentProcessing>(processing->operation);
               }),
           "region attachments compile into reusable material-graph processing nodes");

    for (const auto output : paperweight::materialOutputs) {
        const auto serial = paperweight::generate({
            material, 43, 37, output, std::nullopt, std::nullopt, 1,
        });
        const auto parallel = paperweight::generate({
            material, 43, 37, output, std::nullopt, std::nullopt, 4,
        });
        const auto* serialImage = std::get_if<paperweight::Image>(&serial);
        const auto* parallelImage = std::get_if<paperweight::Image>(&parallel);
        expect(serialImage != nullptr && parallelImage != nullptr &&
                   std::equal(serialImage->pixels().begin(), serialImage->pixels().end(),
                              parallelImage->pixels().begin()),
               "region-attached material output is byte-identical with one and four workers");
    }

    auto invalid = material;
    std::get<paperweight::RegionAttachmentOperation>(
        invalid.layers.back().operation).count = 0;
    expect(paperweight::validateMaterial(invalid).has_value(),
           "invalid attachment counts are rejected before evaluation");
}

void testRegionalSurfaceDetailAndVisualRichness()
{
    paperweight::Material material;
    material.seed = 330033;
    material.physicalSize = {2.4, 1.6};
    material.reliefDepthMetres = 0.012;
    material.metadata = paperweight::MaterialMetadata{
        "33003300-0000-4000-8000-000000000033",
        "Regional Detail Round Trip",
        "A complete physical regional-detail fixture.",
        "showcase",
        {"regional", "surface", "test"},
    };

    auto regions = paperweight::makeCourseLayoutLayer();
    auto& layout = std::get<paperweight::CourseLayoutOperation>(regions.operation);
    layout.profile = paperweight::CourseLayoutProfile::slabs;
    layout.blocks = 5;
    layout.courses = 4;
    layout.gap = 0.035;
    layout.seedOffset = 3301;

    auto detailLayer = paperweight::makeRegionalDetailLayer();
    auto& detail = std::get<paperweight::RegionalDetailOperation>(detailLayer.operation);
    detail.field = paperweight::RegionalDetailField::material;
    detail.orientation = paperweight::RegionalDetailOrientation::region;
    detail.variationScope = paperweight::RegionalVariationScope::parentRegion;
    detail.wearBias = paperweight::RegionalWearBias::mixed;
    detail.macroScaleMetres = 0.42;
    detail.mesoScaleMetres = 0.085;
    detail.microScaleMetres = 0.006;
    detail.macroStrength = 0.31;
    detail.mesoStrength = 0.19;
    detail.microStrength = 0.07;
    detail.gradientStrength = 0.16;
    detail.gradientAngleDegrees = 37.0;
    detail.mottlingStrength = 0.23;
    detail.grainStrength = 0.11;
    detail.strokeStrength = 0.14;
    detail.outerBandMetres = 0.006;
    detail.bevelBandMetres = 0.014;
    detail.innerBandMetres = 0.004;
    detail.edgeIrregularity = 0.42;
    detail.edgeBreakup = 0.18;
    detail.edgeTaper = 0.27;
    detail.wearAmount = 0.36;
    detail.wearScaleMetres = 0.052;
    detail.paletteLow = {45, 52, 58, 255};
    detail.paletteHigh = {176, 168, 148, 255};
    detail.paletteSteps = 5;
    detail.colourAmount = 0.82;
    detail.heightAmount = 0.11;
    detail.roughnessAmount = 0.21;
    detail.coatingWear = 0.63;
    detail.occlusionAmount = 0.29;
    detail.seedOffset = 3302;
    detail.target = paperweight::ProcessingTarget::colourAndScalar;
    material.layers = {regions, detailLayer};

    expect(!paperweight::validateMaterial(material),
           "physical regional surface detail validates");

    const paperweight::RegionSample region{
        0x330033ULL, 0.31, 0.64, 0.42, 0.08, true,
        0x330000ULL, true, 0.125, 0.22, 0.19,
    };
    const auto fields = paperweight::evaluateRegionalDetailFields(
        detail, material, region, 1.0, 0.31, 0.64);
    const auto repeated = paperweight::evaluateRegionalDetailFields(
        detail, material, region, 1.0, 1.31, -0.36);
    const auto bounded = [](double value) {
        return std::isfinite(value) && value >= 0.0 && value <= 1.0;
    };
    expect(fields == repeated,
           "region-local macro, meso, micro, gradients, edges, and wear ignore tile translation");
    expect(bounded(fields.macro) && bounded(fields.meso) && bounded(fields.micro) &&
               bounded(fields.mottling) && bounded(fields.grain) &&
               bounded(fields.directionalStrokes) && bounded(fields.outerShadow) &&
               bounded(fields.bevel) && bounded(fields.body) &&
               bounded(fields.innerHighlight) && bounded(fields.wear) &&
               bounded(fields.palette),
           "every regional surface field remains finite and bounded");
    expect(fields.macro != fields.meso && fields.meso != fields.micro,
           "physical macro, meso, and micro bands remain independently recognisable");

    auto textureDetail = detail;
    textureDetail.orientation = paperweight::RegionalDetailOrientation::texture;
    textureDetail.variationScope = paperweight::RegionalVariationScope::material;
    const auto textureFields = paperweight::evaluateRegionalDetailFields(
        textureDetail, material, {}, 1.0, 0.173, 0.291);
    const auto textureRepeated = paperweight::evaluateRegionalDetailFields(
        textureDetail, material, {}, 1.0, 1.173, -0.709);
    expectNear(textureFields.directionalGradient,
               textureRepeated.directionalGradient, 1.0e-12,
               "texture-space directional gradients close exactly on the torus");
    expectNear(textureFields.planarGradient,
               textureRepeated.planarGradient, 1.0e-12,
               "texture-space planar gradients close exactly on the torus");
    expectNear(textureFields.grain, textureRepeated.grain, 1.0e-12,
               "arbitrarily directed texture grain uses tile-compatible winding");
    expectNear(textureFields.directionalStrokes,
               textureRepeated.directionalStrokes, 1.0e-12,
               "broken directional strokes remain mathematically seamless");

    auto raw = detail;
    raw.field = paperweight::RegionalDetailField::grain;
    raw.target = paperweight::ProcessingTarget::colour;
    const paperweight::EvaluatedSample input{0.2, 0.3, 0.4, 0.71, 1.0, region};
    const auto routed = paperweight::evaluateRegionalDetail(
        raw,
        paperweight::EvaluationContext{
            material, 0.31, 0.64, paperweight::MaterialOutput::height},
        input);
    expect(routed.scalar == input.scalar && routed.red == routed.green &&
               routed.green == routed.blue,
           "raw detail masks can target colour independently from physical channels");

    const auto encoded = paperweight::serialisePmat(material);
    const auto* text = std::get_if<std::string>(&encoded);
    expect(text != nullptr && text->find("pmat.version = 24") != std::string::npos &&
               text->find("operation = regional_detail") != std::string::npos &&
               text->find("detail.macro_scale = 0.42m") != std::string::npos &&
               text->find("detail.variation_scope = parent_region") != std::string::npos &&
               text->find("detail.palette_steps = 5") != std::string::npos,
           "PMAT 24 stores the complete regional-detail vocabulary readably");
    if (text != nullptr) {
        const auto decoded = paperweight::parsePmat(*text);
        const auto* roundTripped = std::get_if<paperweight::Material>(&decoded);
        expect(roundTripped != nullptr && *roundTripped == material,
               "PMAT 24 round-trips regional surface detail exactly");
        const auto rejected = paperweight::parsePmat(
            downgradeCurrentPmat(*text, 23));
        const auto* diagnostic = std::get_if<paperweight::ParseDiagnostic>(&rejected);
        expect(diagnostic != nullptr && diagnostic->message.find("version 24") !=
                   std::string::npos,
               "PMAT 23 explicitly rejects regional surface detail");
    }
    ...
    expect(graph != nullptr && std::any_of(
               graph->nodes.begin(), graph->nodes.end(), [](const auto& node) {
                   const auto* processing = std::get_if<paperweight::ProcessingNode>(&node);
                   return processing != nullptr && std::holds_alternative<
                       paperweight::RegionalDetailProcessing>(processing->operation);
               }),
           "regional detail compiles into a reusable graph processing object");

    auto grouped = material;
    grouped.layerGroups = {paperweight::MaterialLayerGroup{
        "group.stone", {}, "Stone", true, 1.0,
        paperweight::CompositeMode::blend, {}, {}, {}}};
    grouped.layerHierarchy = {
        {"layer.regions", {}},
        {"layer.detail", "group.stone"},
    };
    const auto groupKey = [](const paperweight::Material& groupedMaterial) {
        const auto result = paperweight::compileMaterialGraph(groupedMaterial);
        const auto* compiledGraph = std::get_if<paperweight::MaterialGraph>(&result);
        if (compiledGraph == nullptr) return std::uint64_t{};
        for (const auto& node : compiledGraph->nodes) {
            const auto* processing = std::get_if<paperweight::ProcessingNode>(&node);
            if (processing == nullptr) continue;
            const auto* regional = std::get_if<paperweight::RegionalDetailProcessing>(
                &processing->operation);
            if (regional != nullptr) return regional->groupScopeKey;
        }
        return std::uint64_t{};
    };
    const auto stoneGroupKey = groupKey(grouped);
    grouped.layerGroups.front().identity = "group.wood";
    grouped.layerHierarchy.back().parentGroupIdentity = "group.wood";
    const auto woodGroupKey = groupKey(grouped);
    expect(stoneGroupKey != 0 && woodGroupKey != 0 && stoneGroupKey != woodGroupKey,
           "group-scoped variation derives from the stable containing-group identity");

    for (const auto output : paperweight::materialOutputs) {
        const auto serial = paperweight::generate({
            material, 67, 53, output, std::nullopt, std::nullopt, 1,
        });
        const auto parallel = paperweight::generate({
            material, 67, 53, output, std::nullopt, std::nullopt, 4,
        });
        const auto* serialImage = std::get_if<paperweight::Image>(&serial);
        const auto* parallelImage = std::get_if<paperweight::Image>(&parallel);
        expect(serialImage != nullptr && parallelImage != nullptr &&
                   std::equal(serialImage->pixels().begin(), serialImage->pixels().end(),
                              parallelImage->pixels().begin()),
               "regional detail is byte-identical with one and four workers");
    }

    auto invalid = material;
    std::get<paperweight::RegionalDetailOperation>(
        invalid.layers.back().operation).microScaleMetres = 0.0;
    expect(paperweight::validateMaterial(invalid).has_value(),
           "zero physical detail scale is rejected before evaluation");
    invalid = material;
    std::get<paperweight::RegionalDetailOperation>(
        invalid.layers.back().operation).paletteSteps = 1;
    expect(paperweight::validateMaterial(invalid).has_value(),
           "a one-step regional palette is rejected before evaluation");
}
...
int main()
{
    ...
    testLayerEvaluation();
    testLayerFragments();
    testLayerGroupsAndReusableOverlays();
    testMasksAndWarping();
    ...
    testOrganicSilhouettesAndHierarchicalClusters();
    testTextilesAndFibres();
    testRegionAttachedDetailAndDamage();
    testRegionalSurfaceDetailAndVisualRichness();
