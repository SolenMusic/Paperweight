...
Paperweight v0.0.33 reads `.pmat` format versions 1 through 24 and writes version 24.
...
`RegionalDetailOperation` is a processing node, not a new source generator. It
consumes the current sample and its optional `RegionSample`, derives all detail
signals from material-space coordinates and stable hashes, then either exposes
one raw field or applies a coordinated material treatment. Region producers
publish approximate UV extents so metre-authored frequencies and edge widths do
not depend on output resolution.

Macro, meso, micro, wear, contour breakup, palette variation, and gradients all
share the material seed plus an explicit seed offset. No signal depends on pixel
iteration, worker scheduling, or mutable random state. Texture-space signals
wrap on the torus; region-space signals use stable local coordinates and
orientation. See [regional-surface-detail.md](regional-surface-detail.md).
...
In v0.0.32 each editor owns a native multi-select layer outline. Flat v0.0.31
documents may continue using transient UUIDs, but a grouped material persists
stable group and layer identities plus parent relationships in `Material` and
PMAT 23. Identities define topology only; random variation remains derived from
the material seed and authored seed offsets. The visible outline presents
higher evaluation entries first and disclosure state remains editor-only.

Cross-document transfer uses the portable `layer_fragment` codec. Its versioned
text envelope carries either a lossless ordered layer subset or a complete group
subtree through a dedicated pasteboard type, while an ordinary text
representation remains available for inspection. Carrier-level PMAT metadata is
ignored, so paste cannot replace the destination seed, name, UID, physical
repeat, or surface settings. The AppKit controller remaps every persistent
identity before inserting the fragment.

The graph compiler builds a tree from the validated flat evaluation vector and
parent identities. Each group evaluates its descendants against the accumulated
input below the group, then applies the group's route, mask, composite mode, and
opacity once to the complete result. This pass-through rule makes a default
group byte-preserving while letting one mask sample cover all selected output
branches coherently. `.pwoverlay` is a seedless metadata envelope around the
same portable group fragment, not a second evaluator or a live external link.

Structural stack changes capture layers, groups, hierarchy, identities, and
selection in a bounded per-editor `NSUndoManager`. Parameter edits clear that structural history until
...
outside v0.0.32. MetalKit presents CPU-generated images with metallic/roughness,
