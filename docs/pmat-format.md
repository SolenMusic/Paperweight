...
- `groups.count` may be from 0 to 32. A grouped material gives every layer and
  group a unique stable identity and stores `none` for a root parent.
...
| --- | --- | --- |
| `pmat.version` | File-format version | `1` through `23`; the serialiser writes `23` |
...
| `groups.count` | Number of portable layer groups | Integer from 0 to 32 |
...
required in versions 3 through 23 even when their optional features are disabled; this
...
Paperweight v0.0.33 retains the layer syntax, now at version 24, as the compact,
...
- levels, threshold, shape Boolean, surface filters, posterise, colour ramps,
  palettes, ink contours, region fields, region surfaces, region attachments,
  and regional surface detail become unary
  processing nodes;
...
for future diagnostics and incremental evaluation. Version-23 layers compile
...
Format version 23 adds portable layer groups. `groups.count` is always explicit;
each `group.N` stores a stable identity, optional parent, friendly name, enabled
state, composite mode, opacity, output routing, transform, and periodic mask.
Grouped layers also store `layer.N.id` and `layer.N.parent`. A default blend
group is pass-through and byte-preserving. The validator rejects missing
parents, duplicate identities, cycles, nesting beyond eight levels, and split
group ranges. Versions 1 through 22 acquire `groups.count = 0` and remain flat,
retaining every historical output byte.

Format version 24 adds `regional_detail`, a reusable processing operation whose
physical scales and edge-band widths use metre-suffixed values. Its field can be
`material`, `macro`, `meso`, `micro`, `centre_gradient`,
`directional_gradient`, `planar_gradient`, `mottling`, `grain`,
`directional_strokes`, `outer_shadow`, `bevel`, `body`, `inner_highlight`,
`wear`, `combined`, or `palette`. Orientation can use the texture or the stable
local frame of the current region; variation can be keyed per material,
containing layer-group identity, parent region, or region. Wear may favour exposed edges, cavities, upward faces,
local patches, or a mixed signal. Raw fields obey the standard processing
target, while material mode coordinates colour, height, roughness, coating,
clear-coat roughness, and occlusion from the same deterministic signals.

```text
layer.2.operation = regional_detail
layer.2.detail.field = material
layer.2.detail.orientation = region
layer.2.detail.variation_scope = region
layer.2.detail.wear_bias = mixed
layer.2.detail.macro_scale = 0.24m
layer.2.detail.meso_scale = 0.055m
layer.2.detail.micro_scale = 0.008m
layer.2.detail.outer_band = 0.004m
layer.2.detail.bevel_band = 0.009m
layer.2.detail.inner_band = 0.003m
```

All regional-detail parameters are explicit in version 24. Versions 1 through
23 contain no such operation and retain their previous generated bytes.
...
Paperweight v0.0.33 reads versions 1 through 24 and writes version 24. A reader
...
migration to version 24. Version 18 adds opt-in special-surface branches and
...
Version 23 adds opt-in group topology and stable identities. Version-22 and
older materials load with no groups, so their graph and generated bytes remain
unchanged.
Version 24 adds opt-in regional surface detail. Version-23 and older materials
contain no such layer, so their graph and generated bytes remain unchanged.
...
The editor clipboard has a separately versioned layer-fragment envelope. Version
1 remains readable for v0.0.31 flat selections. Version 2 can carry a complete
v0.0.32 group subtree, including stable identities and parent relationships.
Paste remaps those identities. Material identity, seed, physical size, and
surface-wide values remain properties of the destination document.

`.pwoverlay` version 1 adds an identifier, friendly name, and description around
a version-2 group fragment. It stores no seed and is therefore reusable across
materials without becoming a linked dependency.
